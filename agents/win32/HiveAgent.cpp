...
extern json connect_to_group(const std::string &ssid);
extern json create_wifi_direct_group(const std::string& device_id, bool ruthless_mode);
extern json stop_wifi_direct_group();
extern json create_tethering_hotspot(const std::string& device_id, const std::string& passphrase);
extern json stop_tethering_hotspot();
extern json perform_startup_restoration();
...
void cleanup_on_exit() {
    ...
    try { stop_tethering_hotspot(); } catch (...) {}
}
...
void handle_command(const json &cmd) {
    try {
        ...
        if (type == "STATUS") send_status(agent_state_to_string(g_current_agent_state.load()), "Agent operational");
            ...
            json res = connect_to_group(cmd.value("uuid", cmd.value("ssid", "")));
            ...
            json res = create_wifi_direct_group(cmd.value("device_id", "WIN"), cmd.value("ruthless_mode", false));
            ...
            g_current_agent_state = AgentState::DISCONNECTED;
        } else if (type == "CREATE_HOTSPOT") {
            json res = create_tethering_hotspot(cmd.value("device_id", "WIN"), cmd.value("passphrase", ""));
            if (res["type"] == "HOTSPOT_CREATED") g_current_agent_state = AgentState::CONNECTED_AS_GO;
            send_json_response(res);
        } else if (type == "STOP_HOTSPOT") {
            send_json_response(stop_tethering_hotspot());
            g_current_agent_state = AgentState::DISCONNECTED;
...
int main(int argc, char *argv[]) {
    ...
    if (g_interactive_mode) fprintf(stderr, "Hive Agent (Win32) v%s\n", g_agent_version);
